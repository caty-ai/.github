...
We also measure whether the machinery actually works. In a sealed, pre-registered, machine-scored benchmark (Claude Haiku 4.5, 2026-08; both arms limited to read and write tools, no search), the same model verifiably finished **13%** of context-overflow jobs on its own, and **43%** with the harness driving it. On jobs small enough to fit comfortably, the harness gave no advantage — we publish that too, along with the failures it still has: [the full numbers and limitations](https://github.com/caty-ai/caty-agent-harness/blob/main/docs/benchmark.md). Which models its overflow sentinel helps — and where it should not be switched on — are measured and published under the same sealed discipline: [per-model activation profiles](https://github.com/caty-ai/caty-agent-harness/blob/main/docs/benchmark.md#ev-008).
