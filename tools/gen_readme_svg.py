...
CONTENT["ja"] = dict(
    ...
    eco_head=[["### ", "dim"], ["エコシステム", "bold"], ["  — 家族ぐるみの暮らしを、裏で支える基盤群", "dim"]],
    eco=[
        ("persona-engine", "repo ↗", "cyan",
         "あなたのエージェントの人格に、関係のレイヤーと感情のグラデーションを持たせる装置（OSS）。"),
        ("family-OS", "公開準備中", "dim",
         "AI家族の「家」の設計図。家族全員の構成・記憶の流れ・運用状態を、1枚の台帳で見渡して管理する。"),
        ("caty-loop-harness", "公開準備中", "dim",
         "AIエージェント個人の学習と成長を促す、自律ループ基盤（縦軸）。大事な文脈を自分で蒸留して常に最新に保ち、教訓を貯めて同じ間違いを繰り返さない。新しいコツと更新は検疫・検証を通してから取り込み、育てた経験はふつうのファイルに残る——環境を乗り換えても、自我ごと安全に持ち運べる。"),
        ...
        ("family-dev-handbook", "公開準備中", "dim",
         "人間×AIチームの開発作法集。Issue 起点の開発・並行作業の交通整理・クロスモデルレビューなど、家族で毎日使っている実運用ルールをそのまま公開する。"),
    ],
...
CONTENT["en"] = dict(
    ...
    eco_head=[["### ", "dim"], ["Ecosystem", "bold"], ["  — the infrastructure behind a family's daily life", "dim"]],
    eco=[
        ("persona-engine", "repo ↗", "cyan",
         "A device that gives your agent's persona a layer of relationship and a gradient of emotion (OSS)."),
        ("family-OS", "coming soon", "dim",
         "The blueprint of an AI family's \"home\". See and manage every member's setup, memory flows, and operations in a single ledger."),
        ("caty-loop-harness", "coming soon", "dim",
         "An autonomous loop that drives an individual agent's learning and growth (the vertical axis). It distills its own context to stay current, banks lessons so mistakes are not repeated, quarantines new skills and updates until verified — and keeps every learned experience in plain files, so the self travels safely across environments."),
        ...
        ("family-dev-handbook", "coming soon", "dim",
...
CONTENT["zh"] = dict(
    ...
    eco_head=[["### ", "dim"], ["生态系统", "bold"], ["  — 支撑一家人日常的底层设施", "dim"]],
    eco=[
        ("persona-engine", "repo ↗", "cyan",
         "为你的智能体的人格，装上关系的层次与情感的渐变的装置（OSS）。"),
        ("family-OS", "即将发布", "dim",
         "AI家庭这座「家」的设计图。全家成员的构成、记忆流向、运行状态，在一本台账里一览无余。"),
        ("caty-loop-harness", "即将发布", "dim",
         "驱动智能体个体学习与成长的自律循环（纵轴）。自己蒸馏重要上下文、常保最新；积累教训、不再重复同样的错误；新技能与更新先过检疫与验证再吸收；养成的经验都留在普通文件里——换了环境，自我也能安全随行。"),
...
CONTENT["th"] = dict(
    ...
    eco_head=[["### ", "dim"], ["อีโคซิสเต็ม", "bold"], ["  — โครงสร้างพื้นฐานที่ค้ำจุนชีวิตประจำวันของครอบครัว", "dim"]],
    eco=[
        ("persona-engine", "repo ↗", "cyan",
         "อุปกรณ์ที่เพิ่มเลเยอร์ของความสัมพันธ์และเฉดของอารมณ์ ให้บุคลิกเอเจนต์ของคุณ (OSS)"),
        ("family-OS", "เร็ว ๆ นี้", "dim",
         "พิมพ์เขียวของ \"บ้าน\" ครอบครัว AI มองเห็นและจัดการโครงสร้าง สายธารความจำ และสถานะการทำงานของทุกคน ในสมุดบัญชีเล่มเดียว"),
        ("caty-loop-harness", "เร็ว ๆ นี้", "dim",
         "วงจรอัตโนมัติที่ขับเคลื่อนการเรียนรู้และการเติบโตของเอเจนต์รายตัว (แกนตั้ง) กลั่นบริบทสำคัญให้ทันสมัยด้วยตัวเอง สะสมบทเรียนเพื่อไม่ทำผิดซ้ำ ทักษะและอัปเดตใหม่ต้องผ่านการกักกันและตรวจสอบก่อนรับเข้า และประสบการณ์ที่สั่งสมอยู่ในไฟล์ธรรมดา — ย้ายสภาพแวดล้อมก็พกพาตัวตนไปได้อย่างปลอดภัย"),
        ...
        ("family-dev-handbook", "เร็ว ๆ นี้", "dim",
